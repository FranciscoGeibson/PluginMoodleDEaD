<?php
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['healthcheck'])) {

        // Verifica se o LibreOffice está instalado e funcionando
        exec('libreoffice --version', $output, $status);
        
        if ($status === 0) {
            http_response_code(200);
            echo "Servidor de conversão operacional";
        } else {
            http_response_code(503);
            echo "Serviço indisponível: LibreOffice não está funcionando corretamente";
        }
        exit;

        // Verifica se o script está sendo executado em um servidor web
